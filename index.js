...
const { checkDataName, createDataId, getDataId, addDataId, removeDataId, checkDataId, checkClaim, getClaim, expiredClaim, addUserClaim, getHit, cmdAdd, expiredCmd } = require("./lib/totalcmd");
const {getDateId, resetLevelingXp, userXp, userLeveling, getLevelingXp, getLevelingLevel, getLevelingId, addLevelingXp, addLevelingLevel, addUserId } = require("./lib/levelup");
...
const exif = new Exif();
 ...
 //******************* LIB FILE ********************\\
const {isUrl, makeid, calculate_age, generateMessageTag, runtime, randomNomor, jsonformat, generateProfilePicture, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./lib/functions')
const { color, bgcolor } = require('./lib/color.js')
const hx = require("./lib/hxz-api")
const { webp2gifFile, igDownloader, TiktokDownloader } = require("./lib/gif.js")
const { fetchJson, getBase64, kyun, createExif } = require('./lib/fetcher')
const { styleText, baseURI, ytsr, yta, ytv, igdl, upload, buffer2Stream, stream2Buffer, ythd, formatDate} = require('./lib/ytdl')
...
const { uploadFile1Day, uptotele, uploadimg, uploadFile, uploadImages } = require('./lib/uploadimage')
...
const { gethitUser, checkHit, AddHit, isLimit, limitAdd, getLimit, giveLimit, addBalance, kurangBalance, getBalance, isGame, gameAdd, givegame, cekGLimit } = require("./lib/limit");
const { bad, menu, tekssalah, katahai, katamalem, katasiang, katasore, katalopyou, teksbanned, tekscmd, teksspam, ilhamberkata, ngebucin, badud, ohayo, salam, thanks, anime} = require("./message/messages.js");
...
const { antivirus } = require('./virtex/antivirus.js')
 ...
 //******************* JSON FILE ********************\\
const { _scommand, commandsDB, setiker, audionye, musiknye, imagenye, videonye, bancht, spamkuyy, senbadword, ban } = require("./message/jsonfile");
const { listcmdblock, _reminder, welkom, settings, listerror, premium, _claim, sewa, user } = require("./message/jsonfile");
const { blocked,hitnya,spamBan,antidel } = require("./message/jsonfile");
...
function mentions(teks, mems = [], id) {
			if (id == null || id == undefined || id == false) {
			  let res = xdev.sendMessage(from, { text: teks, mentions: mems })
			  return res
			} else {
		      let res = xdev.sendMessage(from, { text: teks, mentions: mems }, { quoted: dev })
		      return res
 		    }
		}
...
module.exports = xdev = async (xdev, dev, baterai) => {
	try {
        ...
        const isClaimOn = checkClaim(senderNumber, _claim)
...
const isMuted = checkDataId ("mute", from, DataId) 
...
const isNsfw = checkDataId ("nsfw", from, DataId) 
...
xdev.sendMessage(from, teks, text, { quoted: dev, contextInfo: { forward, externalAdReply:{title: fake,mediaType:"2", thumbnail: fs.readFileSync('./stik/fake.jpeg'), mediaUrl:`https://youtu.be/uQiF1yOnzDg`}}})
} 
}
...
let _p = []
let _vote = '*Absen* '+ '@'+ _votes[0].votes.split('@')[0] + `\n\n*Alasan*: ${_votes[0].reason}\n*Jumlah Member* : ${vote.length} Member\n*Durasi* : ${_votes[0].durasi} Menit\n\n` 
...
if(total >= 0){
await setReply(`Berhasil mengusir ${total} orang asing dari grup.`)
}else {
await setReply(`Di grup ini tidak ada orang asing.`)
}
break
*/
...
case 'setquoted':
	...
	if ((args[0]) === 'ftoko'|| (args[0]) === 'product' ){
        ...
        fakeTeks = false   
        fakeTroli = false
		await xdev.sendMessage(from,"Berhasil mengubah quoted ke ftoko", text, {quoted: ftoko})
        ...
        fakeTeks = false   
        fakeTroli = false
		await xdev.sendMessage(from,"Berhasil mengubah quoted ke fkontak", text, {quoted: fkontak})
        ...
        fakeTeks = true   
        fakeTroli = false
		await xdev.sendMessage(from,"Berhasil mengubah quoted ke ftext", text, {quoted: ftext})
        ...
        fakeTeks = false   
        fakeTroli = true
		await xdev.sendMessage(from,"Berhasil mengubah quoted ke ftroli", text, {quoted: ftroli})
		} else if (!q) {
...
break
                      ...
                      case 'joox':
const istheUrl = str => /https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/gi.test(str)
    if (!q) setReply( `*Perintah ini untuk mencari lagu joox berdasarkan pencarian*\n\ncontoh:\n${prefix + command} akad`)
    if (istheUrl(q)) setReply( `*Perintah ini untuk mencari lagu joox berdasarkan pencarian bukan link*\n\ncontoh:\n${prefix + command} akad`)
    let json = await joox(q)
     result = json.data[Math.floor(Math.random() * json.data.length)]
    let pesan = `
...
`.trim()
    ...
    sendFileFromUrl(result.img, image,  {caption: pesan, thumbnail: result.img })
    sendFileFromUrl( result.mp3, audio, { mimetype: 'audio/mp4' })
break
...
case 'brazzers':
case 'beautiful':
//if (dev.message.extendedTextMessage === null || dev.message.extendedTextMessage === undefined) return  setReply("Tag atau Reply targetnya")
      ...
      case 'wait': case 'whatanime':{
        ...
        case 'tourl': case 'imagetorul': case 'imgtourl': case 'audiotourl': case 'videotourl': case 'stickertourl': case 'stikertourl': case 'tolink':
									...
									if (isQuotedAudio) {
										...
										Okelor = await imgbb(imgbbkey, owgi)
										teks=`]─ 「 *AUDIO TO URL* 」 ─[
...
Url : ${qbc.display_url}`
											setReply(teks)
												...
												setReply(mess.wait)
												owgi = await xdev.downloadAndSaveMediaMessage(ger)
												ffff = await imgbb(imgbbkey, owgi)
												teks=`]─ 「 *VIDEO TO URL* 」 ─[
...
/*
           ...
           case 'makedata':           
           name = "antilink"
           if(!checkDataName(name, from, DataId)) {
           createDataId(name, DataId)
           }
           setReply("Data Created")
           break
           
           case 'addid':
           case 'makeid':
           name = "antilink"
           if(checkDataId (name, from, DataId)) return setReply("ID Sudah ada di database")
           addDataId(from, name, DataId)
           setReply("ID Has been added")
           break
           
           case 'removeid':
           name = "antilink"
           if(!checkDataId (name, from, DataId)) return setReply("ID sudah tidak ada di database")
           removeDataId (name, from, DataId)
           setReply("ID Has been Removed")
           break
*/
 ...
 *Link :* ${res[0].link}
 ...
 _*Tunggu Proses Mengirim Media......*_`
           ...
           if (isLimit(senderNumber, isPremium, isOwner, limitCount, user)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
					...
					owgi = await  xdev.downloadAndSaveMediaMessage(ger)
					anus = await imgbb(imgbbkey, owgi)
...
if ((isMedia && !dev.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  setReply(mess.wait)
  owgi = await xdev.downloadAndSaveMediaMessage(ger)
  anal = await imgbb(imgbbkey, owgi)
 // teks = await upload(owgi)
...
case 'ytmp4':
				    ...
				    .then((a) => {
				     ...
				     sendMediaURL(from, dl_link).catch(() => setReply(mess.Iv)) 
				     limitAdd(senderNumber, user)
				     })		
				     })
				     } catch (err) {
    ...
    case 'buttons1':
     ...
     case 'wasted2':
      try{
            ...
            await xdev.sendMessage(from, rau, image)
            //sendStickerUrl(from, `${anu1}`)
            } else {
...
break
   ...
   case 'anjay':
   lonte = "https://some-random-api.ml/canvas/wasted?avatar=https://i.ibb.co/pn1SsXG/9a8c5d739d5b.jpg"
...
if (Number(oi2) >= 50) return setReply('Kebanyakan!')
    ...
    case 'delabsen':
            ...
            delVote(from)
            setReply('Sukses Menghapus sesi Absen Di Grup Ini')
            break
    case 'absen':
            if(!isGroupAdmins && !dev.key.fromMe) return 
            if(!isGroup) return setReply(mess.only.group)
            if(!q) return setReply('*Absen*\n\n'+ prefix+ 'absen @tag target | reason  | 1 (1 = 1 Menit)')
            if (isVote) return setReply('Sesi Absen Sedang Berlangsung Di Grup Ini')            
            ...
            await mentions('Absen ' +'@'+ id.split('@')[0]+' Di Mulai' +'\n\n' + `ketik absen untuk absen nama \n-Huruf kecil semua\n\nAbsen = ✅\n\nAlasan: ${split[1]}`,[id],true)
...
//Jika ada yg cek prefix bot akan merespon   
		          ...
		          if (isSpammerr) return
                  addSpammer(sender, "10s", spamkuyy)
                  if(multi) return xdev.sendMessage(from, `Baik kak untuk prefix saat ini adalah :「  Multi  」`, text, { quoted: setQuoted, contextInfo: forward })
...
//Jika ada yang tag nomer owner
                  ...
                  if (itsMe) return
		          addSpammer(sender, "10s", spamkuyy)
                  const kta =['*Bentar ya kak nanti juga di bales sama ownerku* 😉','*Tunggu aja kak, entar juga di bales* 😁','*Iya kak, ada apa tag owner aku nih* 🙂','*iya kak sbntar owner masih ngewe aku*👌']
				  const su = kta[Math.floor(Math.random() * kta.length)]
